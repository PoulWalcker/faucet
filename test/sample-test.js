...
describe("NFT", function () {
  ...
  it("Withdraw fails if amount is more than limit", async () => {
    ...
    console.log(await token.balanceOf(faucet.address))
    await expect(faucet.connect(wallet).withdrawToken(token.address, owner, 300)).to.be.revertedWith(
  ...
  it("Withdraw warks fine if enough tokens are on faucet", async () => {

    const owner = "0x9Bde0836d9F7386446a455684571e7694F9d909C";

    await token.mint(faucet.address, Big(299).mul(Math.pow(10, tokenDecimals)).toFixed())
